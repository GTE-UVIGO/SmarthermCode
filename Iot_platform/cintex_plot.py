...
def MTI_train(x_train, y_train,x_test,y_test,med,var,rf_param_grid,h):
    ...
    for u in range(2):
        ...
        model = RandomForestRegressor(n_estimators=grid_rf.best_params_[names_pam[0]],
                                         ...
                                         min_samples_split=grid_rf.best_params_[names_pam[2]], n_jobs=4)
        #model = RandomForestRegressor(n_estimators=grid_rf.best_params_[names_pam[0]], n_jobs=5)
...
def MTI_total(DATA_C, var, poss):
    ...
    predicted_temp = [0 for x in range(2)]
    predicted_hum = [0 for x in range(2)]
    predicted_co2 = [0 for x in range(2)]
    real_temp = [0 for x in range(2)]
    real_hum = [0 for x in range(2)]
    real_co2 = [0 for x in range(2)]
    ...
    #oo = np.concatenate((oo5, oo52))
        ...
        if var == 'temp':
            ...
            #y_predicted1, y_predicted2, rmse1,rmse2  = MTI_train(x_train, y_train, x_test, y_test, med_temp, var,rf_param_grid)
                predicted_temp[h], rmse_temp[h]= MTI_train(x_train, y_train, x_test, y_test, med_temp, var,
                                                                 rf_param_grid,h)
            else:
                #G = G1
                predicted_temp[h], rmse_temp[h]= MTI_train(x_train, y_train, x_test, y_test, med_temp, var,
...
#
            ...
            if h == 0:
                G = []
                # y_predicted1, y_predicted2, rmse1,rmse2  = MTI_train(x_train, y_train, x_test, y_test, med_temp, var,rf_param_grid)
                predicted_hum[h], rmse_hum[h] = MTI_train(x_train, y_train, x_test, y_test, med_hum, var,
                                                           rf_param_grid, h)
            else:
                #G = G1
                predicted_hum[h], rmse_hum[h] = MTI_train(x_train, y_train, x_test, y_test, med_hum, var,
...
#
            ...
            if h == 0:
                G = []
                # y_predicted1, y_predicted2, rmse1,rmse2  = MTI_train(x_train, y_train, x_test, y_test, med_temp, var,rf_param_grid)
                predicted_co2[h], rmse_co2[h] = MTI_train(x_train, y_train, x_test, y_test, med_co2, var,
                                                           rf_param_grid, h)
            else:
                #G = G1
                predicted_co2[h], rmse_co2[h] = MTI_train(x_train, y_train, x_test, y_test, med_co2, var,
...
#
        ...
        if var=='temp':
            rmse=rmse_temp
            y_predicted = predicted_temp
            y_test = real_temp
        elif var=='hum':
            rmse=rmse_hum
            y_predicted = predicted_hum
            y_test = real_hum
        else:
            rmse=rmse_co2
            y_predicted = predicted_co2
            y_test = real_co2

    #return (fig, rmse)
    return (y_test,y_predicted, rmse)
...
dates = pd.date_range("00:00", "23:59", freq="1min").strftime('%H:%M')
y_test_temp,y_predicted_temp, rmse_temp=MTI_total(DATA_C, 'temp', poss_temp)
y_test_hum,y_predicted_hum,rmse_hum=MTI_total(DATA_C, 'hum', poss_hum)
y_test_co2, y_predicted_co2, rmse_co2=MTI_total(DATA_C, 'co2', poss_co2)
...
fig = make_subplots(
            ...
            subplot_titles=("<b>12 January 2021</b><br>[CV(RMSE)="+str(np.round(rmse_temp[0][0],2))+"%]", "<b>26 February 2021</b><br>[CV(RMSE)="+str(np.round(rmse_temp[1][0],2))+"%]","<b>12 January 2021</b><br>[CV(RMSE)="+str(np.round(rmse_hum[0][0],2))+"%]",
                            "<b>26 February 2021</b><br>[CV(RMSE)="+str(np.round(rmse_hum[1][0],2))+"%]","<b>12 January 2021</b><br>[CV(RMSE)="+str(np.round(rmse_co2[0][0],2))+"%]", "<b>26 February 2021</b><br>[CV(RMSE)="+str(np.round(rmse_co2[1][0],2))+"%]"),
...
fig['layout']['xaxis5'].title.update(font=dict(size=23))
fig['layout']['xaxis6'].update( title='hours')
fig['layout']['xaxis6'].title.update(font=dict(size=23))
fig['layout']['yaxis1'].update( title='Temperature [ºC]', range=[7.8,24], dtick=2)
fig['layout']['yaxis2'].update( range=[7.8,24], dtick=2)
fig['layout']['yaxis1'].title.update(font=dict(size=23))
fig['layout']['yaxis3'].update(title='Relative humidity [%]', range=[35,75], dtick=10)
fig['layout']['yaxis4'].update( range=[35,75], dtick=10)
fig['layout']['yaxis3'].title.update(font=dict(size=23))
fig['layout']['yaxis5'].update(title='CO<sub>2</sub> [ppm]',range=[335,700], dtick=100)
fig['layout']['yaxis6'].update( range=[335,700], dtick=100)
fig['layout']['yaxis5'].title.update(font=dict(size=23))


error = 0.3
shading = 'rgba(111, 118, 111, 0.35)'
pos1 = np.array([poss_temp[0][0], poss_temp[1][0]])
predict = 'RF_T_pred'

fig = graphs('Real', predict, 'Sensor_error', fig, pd.Series(y_test_temp[0]), pos1, pd.Series(y_predicted_temp[0]), error, dates, 'red', shading, 0,
             'temp')
pos1 = np.array([poss_temp[0][0], poss_temp[1][1]])
fig = graphs('Real', predict, 'Sensor_error', fig, pd.Series(y_test_temp[1]), pos1, pd.Series(y_predicted_temp[1]), error, dates, 'red', shading, 1,
             'temp')
error = 2
shading = 'rgba(111, 118, 111, 0.35)'
pos1 = np.array([poss_hum[0][0], poss_hum[1][0]])
predict = 'RF_H_pred'

fig = graphs('Real', predict, 'Sensor_error', fig, pd.Series(y_test_hum[0]), pos1, pd.Series(y_predicted_hum[0]), error, dates, 'darkcyan', shading, 0,
                         'hum')
pos1 = np.array([poss_hum[0][0], poss_hum[1][1]])
fig = graphs('Real', predict, 'Sensor_error', fig, pd.Series(y_test_hum[1]), pos1, pd.Series(y_predicted_hum[1]), error, dates, 'darkcyan', shading, 1,
                         'hum')
...
predict = 'RF_CO<sub>2</sub>_pred'
shading = 'rgba(111, 118, 111, 0.35)'
pos1 = np.array([poss_co2[0][0], poss_co2[1][0]])
fig = graphs('Real', predict, 'Sensor_error', fig, pd.Series(y_test_co2[0]), pos1, pd.Series(y_predicted_co2[0]), error, dates, 'darkorange', shading,
                         0, 'co2')
pos1 = np.array([poss_co2[0][0], poss_co2[1][1]])
fig = graphs('Real', predict, 'Sensor_error', fig, pd.Series(y_test_co2[1]), pos1, pd.Series(y_predicted_co2[1]), error, dates, 'darkorange', shading,
                         1, 'co2')
