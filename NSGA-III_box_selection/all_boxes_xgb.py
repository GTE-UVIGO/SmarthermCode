...
def MTI_train_cv(x_original, y_temp, y_hum, y_co2, med_temp, med_hum, med_co2, positX, positY, xgb_params):
    ...
    for u in range(len(positX)):
        ...
        if x_val.shape[0]>5:
            ...
            if x_test1.shape[0]>5:
                ...
                X_scaled_co2 = pd.concat([co2_train1, diss_train1, resto_train1], axis=1)
                X_test1_co2 = pd.concat([co2_test1,diss_test1, resto_test1], axis=1)
                #X_test1_co2_2 = pd.concat([co2_test1_2,diss_test1_2,rad_test1_2, resto_test1_2], axis=1)
                X_val1_co2 = pd.concat([co2_val1,diss_val1,  resto_val1], axis=1)
                ...
                if len(y_val1_temp) > 0 and X_test1_temp.shape[0] > 1 and X_scaled_temp.shape[0] > 1:
                    ...
                    if u_temp==1:
                        ...
                        reg_temp = grid_temp.best_params_[names_param[0]]
                        #eta_temp = grid_temp.best_params_[names_param[0]]
                        mod_temp =  xgb.XGBRegressor(n_estimators=grid_temp.best_params_[names_param[0]],
                                                     kwargs= gpu_dict)
                ...
                if len(y_val1_hum) > 0 and X_test1_hum.shape[0] > 1 and X_scaled_hum.shape[0] > 1:
                    ...
                    if u_hum==1:
                        ...
                        reg_hum = grid_hum.best_params_[names_param[0]]
                        #eta_hum = grid_hum.best_params_[names_param[1]]
                        mod_hum =  xgb.XGBRegressor(n_estimators=grid_hum.best_params_[names_param[0]],
                                                    kwargs= gpu_dict)
                ...
                if len(y_val1_co2) > 0 and X_test1_co2.shape[0] > 1 and X_scaled_co2.shape[0] > 1:
                    ...
                    if u_co2==1:
                        ...
                        reg_co2 = grid_co2.best_params_[names_param[0]]
                        #eta_co2 = grid_co2.best_params_[names_param[1]]
                        mod_co2 =  xgb.XGBRegressor(n_estimators=grid_co2.best_params_[names_param[0]],
                                                    kwargs= gpu_dict)
    ...
    #pred_temp = predictions_temp[minimo]
    #pred_hum = predictions_hum[minimo]
    #pred_co2 = predictions_co2[minimo]
#
    #real_temp = real_temp[minimo]
    #real_hum = real_hum[minimo]
    #real_co2 = real_co2[minimo]

   #pos_x_select = np.array(positX)[minimo]
   #pos_y_select = np.array(positY)[minimo]

    rmse_temp_select = np.array(rmse_temp, dtype=float)
    rmse_hum_select = np.array(rmse_hum, dtype=float)
    rmse_co2_select = np.array(rmse_co2, dtype=float)
    global_error = rmse_co2_select+rmse_temp_select+rmse_hum_select
    ...
    return(global_error,  reg_temp, reg_hum,reg_co2)
...
#
    ...
    xgb_params = {'n_estimators':[10,20,60,100,140,200,240],
    }
    with tensorflow.device(tensorflow.DeviceSpec(device_type="GPU")):
        global_error, reg_temp,reg_hum,reg_co2 = MTI_train_cv(x_final, y_temp, y_hum, y_co2,med_temp, med_hum, med_co2, positX, positY,xgb_params)
...
global_error, reg_temp,reg_hum,reg_co2= MTI_total(DATA_C)
...
print('reg_temp:', reg_temp)
print('reg_hum:', reg_hum)
print('reg_co2:', reg_co2)
print(global_error)
#(28.7, 7.8)


np.savetxt('global_error', global_error)
#np.savetxt('pred_humC', pred_hum)
#np.savetxt('pred_co2C', pred_co2)
#np.savetxt('real_tempC', real_temp)
#np.savetxt('real_humC', real_hum)
#np.savetxt('real_co2C', real_co2)
##np.savetxt('pos_X', pos_x_select)
##np.savetxt('pos_Y', pos_y_select)
#np.savetxt('rmse_errorsC', rmse_total)
#np.savetxt('nmbe_errorsC', nmbe_total)
#
