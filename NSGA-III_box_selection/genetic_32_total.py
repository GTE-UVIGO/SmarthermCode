...
from pymoo.factory import get_problem, get_reference_directions,get_decision_making
...
def XGB(X, y_temp, y_hum, y_co2, med_temp, med_hum, med_co2,n_estimator_temp, n_estimator_hum, n_estimator_co2, box1, box2, box3, box4, box5, box6, box7, box8):
    #xgb_params = {'max_depth':6,'eta': 1,'objective': 'squarederror'}

    name1 = tuple([n_estimator_temp, n_estimator_hum, n_estimator_co2, box1, box2, box3, box4, box5, box6, box7, box8])
    try:
    ...
    #possX = np.array([28.8,44.5, 4,45.5, 17.6,6.9])
    #possY = np.array([10.1,14.4, 11.2,5.6,6.8,4])
    possX = np.array([0.3,27.9,44.5])
    possY = np.array([6.4,14.6, 14.4])
    #possX = np.array([4,7.3, 12.4,44.5, 25,45.5])
    #possY = np.array([3,11.7, 14.7,14.4,14.55,5.6])
    #possX = np.array([9.8, 28.9])
    #possY = np.array([7.8, 1.6])
    ...
    for j in range(len(possX)):
        ...
        X_val1 = X_original.copy().iloc[zz1]
        X_val1 = X_val1.reset_index(drop=True)
        y_val1_temp,y_val1_hum, y_val1_co2  = y_temp.copy().iloc[zz1],y_hum.copy().iloc[zz1],y_co2.copy().iloc[zz1]
        ...
        X_final = X_original.copy().drop(zz1, axis=0)
        X_final= X_final.reset_index(drop=True)
        y1_temp, y1_hum, y1_co2 = y_temp.copy().drop(zz1, axis=0),y_hum.copy().drop(zz1, axis=0),y_co2.copy().drop(zz1, axis=0)
        ...
        if X_val1.shape[0]>5:
            if j==0:
                ...
                zz1 = np.intersect1d(yy1, yy2)
                X_test1 = X_final.copy().iloc[zz1]
                X_test1 = X_test1.reset_index(drop=True)
                y_test1_temp, y_test1_hum, y_test1_co2 = y1_temp.copy().iloc[zz1],y1_hum.copy().iloc[zz1],y1_co2.copy().iloc[zz1]
            ...
            if X_test1.shape[0]>5:
                ...
                temperaturas_train = np.array(X_final.copy().iloc[:, np.array([1, 4, 7, 10, 13, 16, 19, 22, 26, 31, 34, 37, 40, 43, 46, 49, 52])])
                temperaturas_test = np.array(X_test1.copy().iloc[:, np.array([1, 4, 7, 10, 13, 16, 19, 22, 26, 31, 34, 37, 40, 43, 46, 49, 52])])
                temperaturas_val = np.array(X_val1.copy().iloc[:, np.array([1, 4, 7, 10, 13, 16, 19, 22, 26, 31, 34, 37, 40, 43, 46, 49, 52])])

                humedad_train = np.array(X_final.copy().iloc[:, np.array([0, 3, 6, 9, 12, 15, 18, 21, 24, 30, 33, 36, 39, 42, 45, 48, 51])])
                humedad_test = np.array(X_test1.copy().iloc[:, np.array([0, 3, 6, 9, 12, 15, 18, 21, 24, 30, 33, 36, 39, 42, 45, 48, 51])])
                humedad_val = np.array(X_val1.copy().iloc[:, np.array([0, 3, 6, 9, 12, 15, 18, 21, 24, 30, 33, 36, 39, 42, 45, 48, 51])])

                co2_train = np.array(X_final.copy().iloc[:, np.array([2, 5, 8, 11, 14, 17, 20, 23, 32, 35, 38, 41, 44, 47, 50, 53])])
                co2_test = np.array(X_test1.copy().iloc[:, np.array([2, 5, 8, 11, 14, 17, 20, 23, 32, 35, 38, 41, 44, 47, 50, 53])])
                co2_val = np.array(X_val1.copy().iloc[:, np.array([2, 5, 8, 11, 14, 17, 20, 23, 32, 35, 38, 41, 44, 47, 50, 53])])

                diss_train = np.array(X_final.copy().iloc[:, range(X_final.shape[1] - 24, X_final.shape[1])])
                diss_test = np.array(X_test1.copy().iloc[:, range(X_final.shape[1] - 24, X_final.shape[1])])
                diss_val = np.array(X_val1.copy().iloc[:, range(X_final.shape[1] - 24, X_final.shape[1])])
                rad_train = np.array(X_final.copy().iloc[:, np.array([25])])
                rad_test = np.array(X_test1.copy().iloc[:, np.array([25])])
                rad_val = np.array(X_val1.copy().iloc[:, np.array([25])])
                resto_train = X_final.copy().iloc[:, np.array([27, 28, 29])]
                resto_test = X_test1.copy().iloc[:, np.array([27, 28, 29])]
                resto_val = X_val1.copy().iloc[:, np.array([27, 28, 29])]
                ...
                y_scaled_temp = pd.DataFrame(scalar_temp.transform(np.array(y1_temp.copy()).reshape(-1, 1)))
                y_scaled_hum = pd.DataFrame(scalar_hum.transform(np.array(y1_hum.copy()).reshape(-1, 1)))
                y_scaled_co2 = pd.DataFrame(scalar_co2.transform(np.array(y1_co2.copy()).reshape(-1, 1)))
                y_test1_temp = pd.DataFrame(scalar_temp.transform(np.array(y_test1_temp.copy()).reshape(-1, 1)))
                y_test1_hum = pd.DataFrame(scalar_hum.transform(np.array(y_test1_hum.copy()).reshape(-1, 1)))
                y_test1_co2 = pd.DataFrame(scalar_co2.transform(np.array(y_test1_co2.copy()).reshape(-1, 1)))
                ...
                if np.sum([box1, box2, box3, box4, box5, box6, box7, box8])<5:
                ...
                elif np.sum([box1, box2, box3, box4, box5, box6, box7, box8])>=5 and np.sum([box1, box2, box3, box4, box5, box6, box7, box8])<7:
                    X_scaled_temp, y_scaled_temp =miss_funct(X_scaled_temp, y_scaled_temp,4)
                    X_scaled_hum, y_scaled_hum =miss_funct(X_scaled_hum, y_scaled_hum,4)
                    X_scaled_co2, y_scaled_co2 =miss_funct(X_scaled_co2, y_scaled_co2,4)
                    X_test1_temp, y_test1_temp =miss_funct(X_test1_temp, y_test1_temp,4)
                    X_test1_hum, y_test1_hum =miss_funct(X_test1_hum, y_test1_hum,4)
                    X_test1_co2, y_test1_co2 =miss_funct(X_test1_co2, y_test1_co2,4)
                    X_val1_temp,y_val1_temp=miss_funct(X_val1_temp,y_val1_temp,4)
                    X_val1_hum, y_val1_hum =miss_funct(X_val1_hum, y_val1_hum,4)
                    X_val1_co2, y_val1_co2 =miss_funct(X_val1_co2, y_val1_co2,4)
                else:
                ...
                X_val1co2 = X_val1_co2.reset_index(drop=True)
                ...
                #model_temp = xgb.XGBRegressor(max_depth=m_depth_temp, eta=etaz_temp, kwargs= gpu_dict)
                #model_hum = xgb.XGBRegressor(max_depth=m_depth_hum, eta=etaz_hum, kwargs=gpu_dict)
                #model_co2 = xgb.XGBRegressor(max_depth=m_depth_co2, eta=etaz_co2, kwargs=gpu_dict)
                model_temp = xgb.XGBRegressor(n_estimators=n_estimator_temp,  kwargs= gpu_dict)
                model_hum = xgb.XGBRegressor(n_estimators=n_estimator_hum,  kwargs=gpu_dict)
                model_co2 = xgb.XGBRegressor(n_estimators=n_estimator_co2, kwargs=gpu_dict)

                if len(y_val1_temp)>0 and X_test1_temp.shape[0]>1 and X_scaled_temp.shape[0]>1:
                    model_temp = model_temp.fit(X_scaled_temp, y_scaled_temp,early_stopping_rounds=5,eval_metric="rmse", eval_set=evallist_temp, verbose=False)
                ...
                if len(y_val1_hum)>0 and X_test1_hum.shape[0]>1 and X_scaled_hum.shape[0]>1:
                    model_hum = model_hum.fit(X_scaled_hum, y_scaled_hum,early_stopping_rounds=5,eval_metric="rmse", eval_set=evallist_hum, verbose=False)
                ...
                if len(y_val1_co2)>0 and X_test1_co2.shape[0]>1 and X_scaled_co2.shape[0]>1:
                    model_co2 = model_co2.fit(X_scaled_co2, y_scaled_co2,early_stopping_rounds=5,eval_metric="rmse", eval_set=evallist_co2, verbose=False)
...
class MyProblem(Problem):
    ...
    def __init__(self, X1, y_temp, y_hum, y_co2, med_temp, med_hum, med_co2, contador, **kwargs):
        ...
        super().__init__(n_var=11,
                         n_obj=4,
                         n_constr=1,
                         xl=np.array([2,2,2, 0, 0, 0, 0, 0, 0, 0,0]),
                         xu=np.array([12,12,12, 1, 1, 1, 1, 1, 1, 1,1]),
                         type_var=np.int,
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        f1, f2, f3,f4 = XGB(X1, self.y_temp, self.y_hum, self.y_co2,self.med_temp, self.med_hum, self.med_co2, int(x[0])*20,int(x[1])*20,int(x[2])*20, int(x[3]), int(x[4]),
              int(x[5]), int(x[6]),int(x[7]), int(x[8]), int(x[9]), int(x[10]))
...
def nsga2_individual(X, y_temp, y_hum, y_co2, med_temp, med_hum, med_co2, contador):
    ...
    algorithm = NSGA3(pop_size=100,repair=MyRepair(), eliminate_duplicates=True,
...
#
   ...
   #         self.data["best"].append(algorithm)
...
#
    ...
    struct_T = res.X
    #    obj_erros = res.F[a, :]
    #    struct_errors = res.X[a, :]
    #    obj_equal = res.F[b, :]
    #    struct_equal = res.X[b, :]
    #else:
    #    obj_T = res.F
    #    struct_T = res.X
    #    obj_erros = 0
...
def MTI_train(DATA, date_init,  names, contador):
    ...
    while p1 < len(yearday2):
        ...
        if np.sum(posis2.iloc[indi, 0] - posis2.iloc[indi, 0].iloc[0]) == 0 and np.sum(
            ...
            yyy = y_co22.copy().iloc[indi, 0]
            avr22 = avr2.copy().iloc[indi]
    ...
    #REDUCED
    ##############################################################################
    stop = np.where(yearday == 340)[0][0]
    stop2 = np.where(yearday == 366)[0][len(np.where(yearday == 366)[0])-1]
    D1 = D1.drop(range(stop,stop2), axis=0)
    D1 = D1.reset_index(drop=True)
    yearday = yearday.drop(range(stop), axis=0)
    yearday = yearday.reset_index(drop=True)
    ...
    stop = np.where(yearday == 32)[0][0]
    stop2 = np.where(yearday == 91)[0][len(np.where(yearday == 91)[0])-1]
    D1 = D1.drop(range(stop,stop2), axis=0)
    D1 = D1.reset_index(drop=True)
    yearday = yearday.drop(range(stop), axis=0)
    yearday = yearday.reset_index(drop=True)
...
#
    ...
    #######################################################################################################
    #, device_index="0,1"
    with tensorflow.device(tensorflow.DeviceSpec(device_type="GPU")):
        obj_T, struct_T, res = nsga2_individual(x_final, y_temp, y_hum, y_co2, med_temp, med_hum, med_co2, contador)


    print("Estructuras factibles", struct_T, "y objetivos", obj_T)
    return (obj_T, struct_T,res)
...
obj_T, struct_T,res= MTI_train(variables, date_init, names, contador)

print('El nÃºmero de evaluaciones totales fue', contador)
np.savetxt('solution_X_total.txt', struct_T)
np.savetxt('solution_total.txt', obj_T)
#np.savetxt('solution_X_error.txt', struct_errors)
#np.savetxt('solution_errors.txt', obj_erros)
#np.savetxt('solution_X_equal.txt', struct_equal)
#np.savetxt('solution_equal.txt', obj_equal)

#print('Solución óptima teniendo en cuenta errores más', sol_1)
#print('Solución óptima teniendo en cuenta todo igual', sol_2)

print(f"{round((time() - t) / 3600, 2)} h.")



obj_T = pd.DataFrame(obj_T)
obj_T.columns = ['Temperatures', 'Relative humidity', 'CO2', 'Number of boxes']

struct_T= pd.DataFrame(struct_T)
struct_T.columns =['trees_T', 'trees_H', 'trees_C', 'box1', 'box2', 'box3', 'box4', 'box5',
             'box6', 'box7', 'box8']

obj_T.to_csv ('result_obj.csv', index = False, header=True, sep='?')
struct_T.to_csv ('result_X_.csv', index = False, header=True, sep='?')
...
#list(res.algorithm.callback.data)
#for algorithm in val:
#    values.append(running.notify(algorithm))
i=0
for algorithm in res.history:
    running.notify(algorithm)
    plt.ylabel(r'$\Delta$ f', labelpad=11)
    plt.tight_layout()
    plt.savefig("running_metric_{num}.png".format(num=i));
    i += 1
...
#pet = [e.pop.get("X") for e in res.history]

#SD distance
#SM = [0 for x in range(len(pet)-1)]
#for i in range(len(pet)-1):
#    F1 = pet[i+1]
#    dd = [0 for x in range(len(pet[1])-1)]
#    for t in range(1,len(pet[1])-1):
#        dd[t] = dist.cdist(pet[i+1][t].reshape(1,-1), pet[i+1][t+1].reshape(1,-1))
#    SM[i] = np.sqrt((1/(len(pet[1])-2))* np.sum(dd- np.nanmean(dd)[0]))
#
#plt.figure()
#plt.plot(np.arange(len(pet)-1), np.array(SM)[:,:,0])
#plt.xlabel("Generation")
