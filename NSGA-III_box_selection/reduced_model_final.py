...
def MTI_train_cv(x_original, y_temp, y_hum, y_co2, med_temp, med_hum, med_co2, positX, positY, n_temp, n_hum, n_co2):
    ...
    for u in range(len(positX)):
        ...
        if x_val.shape[0]>5:
            if u==0:
                ...
                y_test_temp, y_test_hum, y_test_co2 =y_temp_t.copy().iloc[zz1], y_hum_t.copy().iloc[zz1], \
                                                      y_co2_t.copy().iloc[zz1]
            ...
            if x_test1.shape[0]>5:
                ...
                model_temp = xgb.XGBRegressor(n_estimators=n_temp,kwargs= gpu_dict)
                model_hum = xgb.XGBRegressor(n_estimators=n_hum,kwargs= gpu_dict)
                model_co2 = xgb.XGBRegressor(n_estimators=n_co2,kwargs= gpu_dict)
...
def MTI_total(DATA_C):
   ...
   #}
    ...
    with tensorflow.device(tensorflow.DeviceSpec(device_type="GPU")):
        global_error= MTI_train_cv(x_final, y_temp, y_hum, y_co2, med_temp, med_hum, med_co2, positX, positY,n_temp, n_hum,  n_co2)
...
np.savetxt('global_error_reduced', global_error)
