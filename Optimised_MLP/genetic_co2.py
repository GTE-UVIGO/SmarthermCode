...
def ANN2(X, y, med, neurons1, neurons2, neurons3):
    ...
    for j in range(3):
        ...
        scalar_co2.fit(np.concatenate((np.concatenate(co2s_train), np.concatenate(co2s_test), np.array(y1), np.array(y_test1))).reshape(-1, 1))
        ...
        y_scaled= pd.DataFrame(scalar_co2.transform(np.array(y1).reshape(-1, 1)))
        ...
        X_scaled = pd.concat([co2s_train1, resto_train1, diss_train1], axis=1)
        ...
        if X_test1.shape[0]>1:
            if neurons1 > 0 and neurons2 == 0 and neurons3 == 0:
                ...
                with warnings.catch_warnings():
                    ...
                    y_pred = scalar_co2.inverse_transform(y_pred)
                    y_real = scalar_co2.inverse_transform(y_test1)
                    accuracy = np.sqrt(metrics.mean_squared_error(y_real, y_pred)) / med
                    # nmbe= MBE(y_real, y_pred,med)
                    # nmbe = r2_score(y_real, y_pred)
                    # nmbe= np.sum(np.array([neurons1, neurons2, neurons3]))
...
#        X1=self.X1
#        f1,f2 = ANN2(X1,self.y,int(x[0]*20), self.ntest, x[1])
#        g1= x[3]+ 2*bool12(x[0])+ bool12(x[1]) - bool12(x[2])-3
#        #out["F"] = np.column_stack([f1, 100-f2])
#        out["F"] = np.column_stack([f1, abs(f2)])
...
def MTI_train(DATA, date_init, names, contador):
    ...
    # temp_obj_T, temp_x_T, temp_obj, temp_x,res= nsga2_individual(x_final, y_final, xxx, yyy, med_temp, contador)
    ...
    co2_obj_T, co2_x_T, co2_obj, co2_x, res = nsga2_individual(x_final, y_final, med_co2, contador)

    # print("Estructuras factibles temperatura", temp_x_T, "y objetivos", temp_obj_T)
    print("Estructuras factibles humedad", co2_x_T, "y objetivo", co2_obj_T)
    # print("Estructuras factibles co2", co2_x_T, "y objetivo", co2_obj_T)
    # print("Estructura optima temperatura", temp_x, "objetivo", temp_obj)
    print("Estructura optima humedad", co2_x, "objetivo", co2_obj)
    ...
    return (co2_obj_T, co2_x_T, co2_obj, co2_x, res)
...
co2_obj_T, co2_x_T, co2_obj, co2_x, res = MTI_train(variables, date_init, names, contador)

print('El nÃƒÂºmero de evaluaciones totales fue', contador)
np.savetxt('temp_obj_total2.txt', co2_obj_T)
np.savetxt('temp_X_total2.txt', co2_x_T)
np.savetxt('temp_obj2.txt', co2_obj)
np.savetxt('temp_X2.txt',co2_x)
