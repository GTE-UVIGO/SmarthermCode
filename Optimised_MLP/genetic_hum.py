...
def ANN2(X, y, med, neurons1, neurons2, neurons3):
    ...
    for j in range(3):
        ...
        scalar_hum.fit(np.concatenate((np.concatenate(humedad_train), np.concatenate(humedad_test),np.array(y1)[:,0], np.array(y_test1)[:,0])).reshape(-1, 1))
        ...
        y_scaled = pd.DataFrame(scalar_hum.transform(np.array(y1).reshape(-1, 1)))
        ...
        #X_scaled = pd.concat([humedad_train1, rad_train1, resto_train1, diss_train1], axis=1)
        ...
        if neurons1 > 0 and neurons2 == 0 and neurons3 == 0:
            ...
            with warnings.catch_warnings():
                ...
                y_pred = scalar_hum.inverse_transform(y_pred)
                y_real = scalar_hum.inverse_transform(y_test1)
...
def MTI_train(DATA, date_init, ntest, names, contador):
    ...
    for t in np.array([3, 9, 12, 15, 18, 21, 24, 27, 30]):
        # for t in np.array([3,9, 12, 15, 18, 21, 24, 27, 30]):
        aa = np.where(np.array(a[:, t], dtype=float) > 875)[0]
  ...
  #dd = dd.reset_index(drop=True)
    ...
    yearday = X.iloc[:, 32]
    #pos#is = pd.concat([X.iloc[:, 0], X.iloc[:, 1]], axis=1)
    ...
    return (hum_obj_T, hum_x_T, hum_obj, hum_x, res)
...
# DATA JOIN
variables = pd.concat([car, pd.DataFrame(pos_x).set_index(car.index),
...
hum_obj_T, hum_x_T, hum_obj, hum_x, res = MTI_train(variables, date_init, ntest, names, contador)

print('El n̼mero de evaluaciones totales fue', contador)
np.savetxt('hum_obj_total2.txt', hum_obj_T)
np.savetxt('hum_X_total2.txt', hum_x_T)
np.savetxt('hum_obj2.txt', hum_obj)
np.savetxt('hum_X2.txt', hum_x)
