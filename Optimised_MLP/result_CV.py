    ...
    if host=='none':
        ...
        for u in range(len(var_name)):
            ...
            query = f"""
                            SELECT mean({var_name[u]}) AS {var_name[u]} FROM {place} 
                            WHERE time > '2020-06-17T11:00:00Z' AND time < '{time_end_str}'  AND "host"='{host}'
                              AND {place2} != '69' GROUP BY time(1m) 
                        """
    ...
    for u in range(len(positX)):
        ...
        if len(yyy)>20:
            ...
            if var == 'temp':
                ...
                scalardist.fit(np.concatenate((np.concatenate(diss_train), np.concatenate(diss_test))).reshape(-1, 1))
                scalar_hum.fit(np.concatenate((np.concatenate(humedad_train), np.concatenate(humedad_test), y_t, y_test)).reshape(-1, 1))
                scalar_co2.fit(np.concatenate((np.concatenate(co2s_train), np.concatenate(co2s_test))).reshape(-1, 1))
                scalar_rad.fit(np.concatenate((np.concatenate(rad_train), np.concatenate(rad_test))).reshape(-1, 1))
                scalarresto.fit(pd.concat([resto_train, resto_test], axis=0))
            ...
            if x_test.shape[0] > 1:
                ...
                if var=='temp':
                    ...
                    y_test2 = np.array(scalar_temp.inverse_transform(y_test))
                else:
                    y_pred = np.array(scalar_hum.inverse_transform(y_pred))
                    y_test2 = np.array(scalar_hum.inverse_transform(y_test))
...
def MTI_total(DATA_C, var):
    ...
    #X_final = X_final.drop(['pos_z', 'pos_x', 'pos_y'], axis=1)
...
#
    ...
    #################################################################################################
    if var=='temp':
        preds = MTI_train_cv(x_train, y_train, 30,20, 0, positX, positY, med_temp,var)
    elif var=='hum':
        MTI_train_cv(x_train, y_train, 110, 10, 0, positX, positY, med_hum,var)
...
#######################################################################################################
MTI_total(DATA_C, 'temp')
#MTI_total(DATA_C, 'hum')
