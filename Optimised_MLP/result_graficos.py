...
def graphs(real,predict,sensor,fig, y_real, ppp, y_predicted, error, dates, colour,shading, h,var):
    ...
    dates1=list(dates)
    y_real = list(y_real)
    y_predicted = list(y_predicted[:,0])
...
def MTI_train(x_train, y_train,x_test,y_test, neurons1, neurons2, neurons3,med,var):
    preds=[]
    variations= [0 for x in range(5)]
    for j in range(5):
        ...
        if len(e)>0:
            ...
            x_testZ = x_test1.drop(e, axis=0)
            x_testZ = x_testZ.reset_index(drop=True)
            y_testZ = y_test1.drop(e, axis=0)

            y_testZ = y_test1
            y_testZ.iloc[e]=np.repeat(0, len(e))
            ...
            if var=='co2':
                ...
                # Train the model
                time_start = time()
                ANN_model.fit(x_train1, y_train1, epochs=1000, validation_data=(x_testZ, y_testZ), callbacks=[es, mc])
            else:
                ANN_model = Sequential()
                # The Input Layer :
                ANN_model.add(
                    Dense(x_train1.shape[1], kernel_initializer='normal', input_dim=x_train1.shape[1],
                          activation='relu'))
                # The Hidden Layers :
                ANN_model.add(Dense(neurons1, kernel_initializer='normal', activation='relu'))
            ...
            if var == 'temp':
                y_pred = np.array(scalar_temp.inverse_transform(y_pred))
                y_test1 = np.array(scalar_temp.inverse_transform(y_testZ))
            elif var == 'hum':
                y_pred = np.array(scalar_hum.inverse_transform(y_pred))
                y_test1 = np.array(scalar_hum.inverse_transform(y_testZ))
            else:
                y_pred = np.array(scalar_co2.inverse_transform(y_pred))
                y_test1 = np.array(scalar_co2.inverse_transform(y_testZ))
            ...
            if var == 'co2':
                ...
                # Train the model
                time_start = time()
                ANN_model.fit(x_train1, y_train1, epochs=1000, validation_data=(x_test1, y_test1), callbacks=[es, mc])
            else:
                ANN_model = Sequential()
                # The Input Layer :
                ANN_model.add(
            ...
            if var=='temp':
                y_pred = np.array(scalar_temp.inverse_transform(y_pred))
                y_test1 = np.array(scalar_temp.inverse_transform(y_test1))
            elif var=='hum':
                y_pred = np.array(scalar_hum.inverse_transform(y_pred))
                y_test1 = np.array(scalar_hum.inverse_transform(y_test1))
            else:
                y_pred = np.array(scalar_co2.inverse_transform(y_pred))
                y_test1 = np.array(scalar_co2.inverse_transform(y_test1))

        preds.append(y_pred)
        variations[j] = 100 * (np.sqrt(metrics.mean_squared_error(y_test1, y_pred)) / med)
        print(100 * (np.sqrt(metrics.mean_squared_error(y_test1, y_pred)) / med))

    ###############################################################################3
    I = np.where(variations == np.min(variations))[0]
    predict = preds[I.reshape(1,-1)[0,0]]
    #predict = preds[I]
    print(predict)

    return(predict, np.array(variations[I.reshape(1,-1)[0,0]]))

def unique21(array):
    uniq, index = np.unique(array, return_index=True)
    return uniq[index.argsort()], index
...
def MTI_total(DATA_C, var,poss,fig):
    ...
    rmse_temp = [0 for x in range(3)]
    rmse_hum = [0 for x in range(3)]
    rmse_co2 = [0 for x in range(3)]
    ...
    for h in range(3):
        ...
        if h==0:
            oo5 = np.where((yearday == 332))[0]
        elif h==1:
            oo5 = np.where((yearday == 42))[0]
        else:
            if var=='temp':
                oo5 = np.where((yearday == 19))[0]
            elif var=='hum':
                oo5 = np.where((yearday == 46))[0]#23
            else:
                oo5 = np.where((yearday == 18))[0]
        ...
        if var == 'temp':
            y_train = temp_train
            y_test = temp_IF
            y_predicted, rmse_temp[h] = MTI_train(x_train, y_train, x_test, y_test, 30, 20, 0, med_temp,var)
            error=0.3
            shading = 'rgba(111, 118, 111, 0.35)'
            pos1 = np.array([poss[0][0], poss[1][h]])
            predict='T_pred'
            fig = graphs('Real',predict,'Sensor_error', fig, y_test, pos1, y_predicted, error, dates, 'red',shading,h,var)
        elif var == 'hum':
            y_train = hum_train
            y_test = hum_IF
            y_predicted, rmse_hum[h]=MTI_train(x_train, y_train, x_test, y_test, 110, 10, 0,med_hum,var)
            error = 2
            shading = 'rgba(111, 118, 111, 0.35)'
            pos1 = np.array([poss[0][0], poss[1][h]])
            predict='H_pred'
            fig = graphs('Real',predict,'Sensor_error',fig, y_test, pos1, y_predicted, error, dates, 'blue',shading,h,var)
        else:
            y_train = co2_train
            y_test = co2_IF
            y_predicted, rmse_co2[h] = MTI_train(x_train, y_train, x_test, y_test,20, 0, 0, med_co2,var)
            error=50
            predict='CO<sub>2</sub>_pred'
            shading = 'rgba(111, 118, 111, 0.35)'
            pos1 = np.array([poss[0][0], poss[1][h]])
            fig = graphs('Real', predict,'Sensor_error', fig, y_test, pos1, y_predicted, error, dates, 'purple',shading,h,var)
...
variables.to_csv(r'', index = False)
#v = pd.read_csv("var.csv")
DATA= variables
...
for h in range(len(date)):
    HOUR[h] = np.round(date[h].hour + date[h].minute / 100,2)
...
for h in range(len(date)):
    yearday[h] = int(date[h].timetuple().tm_yday)

# Day of the week
WEEK = [0 for x in range(len(date))]
for h in range(len(date)):
    WEEK[h] = int(date[h].weekday())
...
poss_temp = [[1,1,1],[1,2,3]]
poss_hum = [[2,2,2],[1,2,3]]
poss_co2 = [[3,3,3],[1,2,3]]
...
fig = make_subplots(
            ...
            subplot_titles=("<b>27 November 2020</b><br>[CV(RMSE)=1.98 %]", "<b>11 February 2021</b><br>[CV(RMSE)=2.09 %]", "<b>19 January 2021</b><br>[CV(RMSE)=4.68 %]", "<b>27 November 2020</b><br>[CV(RMSE)=1.30 %]", "<b>11 February 2021</b><br>[CV(RMSE)=2.14 %] ", "<b>15 February 2021</b><br>[CV(RMSE)=4.66 %]",
                            "<b>27 November 2020</b><br>[CV(RMSE)=6.75 %]", "<b>11 February 2021</b><br>[CV(RMSE)=7.04 %]", "<b>18 January 2021</b><br>[CV(RMSE)=8.58 %]"),
...
fig['layout']['xaxis7'].update( title='hours')
fig['layout']['xaxis7'].title.update(font=dict(size=22))
fig['layout']['xaxis8'].update( title='hours')
fig['layout']['xaxis8'].title.update(font=dict(size=22))
fig['layout']['xaxis9'].update( title='hours')
fig['layout']['xaxis9'].title.update(font=dict(size=22))
fig['layout']['yaxis1'].update( title='Temperature [ºC]', range=[11,24], dtick=2)
fig['layout']['yaxis2'].update( range=[11,24], dtick=2)
fig['layout']['yaxis3'].update( range=[11,24], dtick=2)
fig['layout']['yaxis1'].title.update(font=dict(size=22))
fig['layout']['yaxis4'].update(title='Relative humidity [%]', range=[23,75], dtick=10)
fig['layout']['yaxis5'].update( range=[23,75], dtick=10)
fig['layout']['yaxis6'].update( range=[23,75], dtick=10)
fig['layout']['yaxis4'].title.update(font=dict(size=22))
fig['layout']['yaxis7'].update(title='CO<sub>2</sub> [ppm]',range=[370,800], dtick=100)
fig['layout']['yaxis8'].update( range=[370,800], dtick=100)
fig['layout']['yaxis9'].update( range=[370,800], dtick=100)
fig['layout']['yaxis7'].title.update(font=dict(size=22))
